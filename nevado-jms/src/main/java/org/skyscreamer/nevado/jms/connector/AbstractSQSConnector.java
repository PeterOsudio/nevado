...
public abstract class AbstractSQSConnector implements SQSConnector {
	...
	public void sendMessage(NevadoDestination destination, NevadoMessage message)
			throws JMSException {
		if (destination == null) {
		...
		Map<String, MessageAttributeValue> messageAttributes = readMessageAttributes(message);

		if (destination instanceof NevadoQueue) {
			if (isAsync() && !message.isDisableMessageID()
			...
			String sqsMessageId = sendSQSMessage((NevadoQueue) destination,
	...
	private Map<String, MessageAttributeValue> readMessageAttributes(NevadoMessage message) throws JMSException {
		System.out.println("----------------------AbstractSQSConnector.readMessageAttributes---------");
        ...
        try{
        	for (Enumeration<String> propertyNames = message.getPropertyNames(); propertyNames.hasMoreElements();) {
                ...
                System.out.println("-------------------Read "+ name  + " -> " + obj);
                MessageAttributeValue value = new MessageAttributeValue();
                
                value.setStringValue(obj.toString()); 
	...
	public void sendMessages(NevadoDestination destination,
			List<NevadoMessage> outgoingMessages) throws JMSException {
		for (NevadoMessage message : outgoingMessages) {
			sendMessage(destination, message);
		}
	}

	public NevadoMessage receiveMessage(NevadoConnection connection,
			NevadoDestination destination, long timeoutMs) throws JMSException,
			InterruptedException {
		long startTimeMs = new Date().getTime();
		SQSQueue sqsQueue = getSQSQueue(destination);
		SQSMessage sqsMessage = receiveSQSMessage(connection, destination,
				timeoutMs, startTimeMs, sqsQueue);
		if (sqsMessage != null) {
			_log.info("Received message " + sqsMessage.getMessageId());
		}
		return sqsMessage != null ? convertSqsMessage(destination, sqsMessage,
				false) : null;
	}
	...
	public void resetMessage(NevadoMessage message) throws JMSException {
		String sqsReceiptHandle = (String) message
				.getNevadoProperty(NevadoProperty.SQSReceiptHandle);
	...
	protected abstract void sendSNSMessage(NevadoTopic topic,
			String serializedMessage) throws JMSException;

	protected SQSQueue getSQSQueue(NevadoDestination destination)
			throws JMSException {
		if (destination == null) {
	...
	protected SQSMessage receiveSQSMessage(NevadoConnection connection,
			NevadoDestination destination, long timeoutMs, long startTimeMs,
			SQSQueue sqsQueue) throws JMSException, InterruptedException {
		SQSMessage sqsMessage;
		while (true) {
			if (connection.isRunning()) {
				sqsMessage = sqsQueue.receiveMessage();
				if (sqsMessage != null && !connection.isRunning()) {
					// Connection was stopped while the REST call to SQS was
					// being made
					try {
						sqsQueue.setMessageVisibilityTimeout(
								sqsMessage.getReceiptHandle(), 0); // Make it
																	// immediately
																	// available
																	// to the
																	// next
						...
						String exMessage = "Unable to reset visibility timeout for message: "
								+ e.getMessage();
						_log.warn(exMessage, e); // Non-fatal. Just means the
		...
		if (_log.isDebugEnabled()) {
			_log.debug("Received message: "
					+ ((sqsMessage != null) ? sqsMessage.getMessageBody()
							: null));
		}
		return sqsMessage;
	}

	protected String sendSQSMessage(NevadoQueue queue,
		...
		return sqsQueue.sendMessage(serializedMessage, messageAttributes);
	}

	protected NevadoMessage convertSqsMessage(NevadoDestination destination,
			SQSMessage sqsMessage, boolean readOnly) throws JMSException {
		// Get the message
		...
		if (destination instanceof NevadoQueue) {
			...
			try {
				messageBody = new JSONObject(sqsMessage.getMessageBody())
						.getString("Message");
			} catch (JSONException e) {
				throw new JMSException(
						"Unable to parse JSON from message body: "
								+ sqsMessage.getMessageBody());
			}
		}
		try {
		...
		if (message.nevadoPropertyExists(NevadoProperty.DisableMessageID)
				&& (Boolean) message
						.getNevadoProperty(NevadoProperty.DisableMessageID)) {
		...
		message.setNevadoProperty(NevadoProperty.SQSReceiptHandle,
	 ...
	 */
	protected String serializeMessage(NevadoMessage message)
			throws JMSException {
		String serializedMessage;
		try {
			serializedMessage = SerializeUtil.serializeToString(message);
		} catch (IOException e) {
			String exMessage = "Unable to serialize message of type "
	 ...
	 *             from the source
	 */
	protected NevadoMessage deserializeMessage(String serializedMessage)
			throws JMSException {
		Serializable deserializedObject;
		try {
			try {
				deserializedObject = SerializeUtil
						.deserializeFromString(serializedMessage);
			...
			String exMessage = "Unable to deserialized message: "
					+ e.getMessage();
			_log.error(exMessage, e);
			throw new JMSException(exMessage);
		}
		...
		if (!(deserializedObject instanceof NevadoMessage)) {
			throw new JMSException(
					"Expected object of type NevadoMessage, got: "
	...
	private String getSQSReceiptHandle(NevadoMessage message)
			throws JMSException {
		String sqsReceiptHandle = (String) message
				.getNevadoProperty(NevadoProperty.SQSReceiptHandle);
		if (sqsReceiptHandle == null) {
