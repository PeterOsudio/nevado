...
public abstract class AbstractSQSConnector implements SQSConnector {
    ...
    public void sendMessage(NevadoDestination destination, NevadoMessage message) throws JMSException
    {
        ...
        Map<String, MessageAttributeValue> attributes = readMessageAttributes(message);
        ...
        if (destination instanceof NevadoQueue)
        {
            ...
            String sqsMessageId = sendSQSMessage((NevadoQueue)destination, serializedMessage, attributes);
    ...
    private Map<String, MessageAttributeValue> readMessageAttributes(NevadoMessage message) throws JMSException {
        ...
        try{
        	for (Enumeration<String> propertyNames = message.getPropertyNames(); propertyNames.hasMoreElements();) {
                ...
                // We might need to extend this later to also enable number and binary types
                value.setStringValue(obj.toString()); 
                value.setDataType("String");
                messageAttributes.put(name, value);
			}
        }catch (JMSException e) {
        	_log.error("Unable to read message attributes: " + e.getMessage());
		}
    ...
    protected String sendSQSMessage(NevadoQueue queue, String serializedMessage, Map<String, MessageAttributeValue> attributes) throws JMSException
    {
        SQSQueue sqsQueue = getSQSQueue(queue);
        if (_log.isDebugEnabled())
        {
        ...
        return sqsQueue.sendMessage(serializedMessage, attributes);
    }
